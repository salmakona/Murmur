<template>
    ...
    <div>
      ...
      <p class="text-justify"><img src="/icons8-calendar-24.png" /> Joined {{join_date}} </p>
      <h6>Followers <span class="badge badge-secondary">{{this.followingcount}}</span></h6>
      <h6>Following <span class="badge badge-secondary">{{this.followersCount}}</span></h6>
    </div>
    ...
    data() {
      ...
      async created(){
        ...
        const resfollowerCount = await axios.get('http://localhost:3001/api/followerCount/'+this.userId );
         this.followersCount = resfollowerCount.data[0]
